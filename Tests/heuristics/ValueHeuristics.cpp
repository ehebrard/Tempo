...
struct TestValueHeuristic : public tempo::heuristics::BaseValueHeuristic<TestValueHeuristic> {
    bool called = false;

    explicit TestValueHeuristic(double epsilon) : tempo::heuristics::BaseValueHeuristic<TestValueHeuristic>(epsilon) {}

    template<typename Sched>
    auto choose(tempo::var x, const Sched &) {
        called = true;
        return tempo::POS(x);
    }
};
...
struct DummyScheduler {
    ...
    auto getEdge(tempo::lit lit) const -> tempo::DistanceConstraint<int> {
        return lit % 2 == 0 and not invert ? tempo::DistanceConstraint{10, 20, 6} : tempo::DistanceConstraint{11, 21, 2};
    }

    int upper(tempo::event e) const {
        return e % 2 == 0 ? 20 : 10;
    }

    int lower(tempo::event e) const {
        return e % 2 == 0 ? 0 : 8 ;
    }
};
