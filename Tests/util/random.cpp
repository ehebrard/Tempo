...
TEST(random, sampler_ctor_pdf) {
    ...
    sampler = TestSampler(std::vector{0.5, 0.75, 0.25}, 0, 100);
    EXPECT_EQ(sampler.getCDF(), (std::vector<unsigned long>{50, 125, 150}));
    EXPECT_THROW(TestSampler(std::vector{1, 4, -1, 2, 9}), std::invalid_argument);
}

TEST(random, sampler_ctor_smoothing) {
    const std::vector<double> pdf{1, 2, 4, 1};
    TestSampler sampler(pdf);
    EXPECT_EQ(sampler.getCDF(), (std::vector<unsigned long>{1, 3, 7, 8}));
    sampler = TestSampler(pdf, 1);
    EXPECT_EQ(sampler.getCDF(), (std::vector<unsigned long>{1, 2, 3, 4}));
    sampler = TestSampler(pdf, 0.5, 10);
    EXPECT_EQ(sampler.getCDF(), (std::vector<unsigned long>{15, 35, 65, 80}));
    EXPECT_THROW(TestSampler(pdf, 1.2), std::invalid_argument);
}

template<tempo::detail::scalar_range PDF, typename T, typename DrawFun>
