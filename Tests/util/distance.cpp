...
TEST(util, distance_bound_estimation_null_edge) {
    ...
    using namespace tempo::testing;
    heuristics::LitProvider provider(makeBooleanLiteral<int>(true, 0, 3), {9, 7, 7, 4}, {8, 7, 3, 3});
    EXPECT_TRUE(std::isnan(boundEstimation(DistanceConstraint{Constant::NoVar, 2, 0}, provider)));
    EXPECT_TRUE(std::isnan(boundEstimation(DistanceConstraint{Constant::NoVar, 2, 0.3}, provider)));
    EXPECT_TRUE(std::isnan(boundEstimation(DistanceConstraint{Constant::NoVar, 2, 0.3f}, provider)));
}
