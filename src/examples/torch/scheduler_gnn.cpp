...
int main(int argc, char **argv) {
    ...
    auto opt = cli::parseOptions(argc, argv,
                                 ...
                                 cli::ArgSpec("gnn-max-fail-ratio", "maximum fail ratio before inference is rerun",
                                              false, maxFailRatio, 0.2));
    ...
    GNNH valBranching(profiler, opt.polarity_epsilon, gnnLocation, featureExtractorConf, *solver, maxFailRatio,
                      std::move(problem));
