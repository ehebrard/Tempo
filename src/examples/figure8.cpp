...
int main() {
  ...
  Solver<> S;
    ...
    auto schedule = S.newInterval(0, 13, 0, 0, 0, 13);

    auto sx{S.newNumeric()};
    auto x{S.between(sx, sx + 1)};
    auto dx{S.newNumeric(1, 1)};
    
    auto sy{S.newNumeric()};
    auto y{S.between(sy, sy + 2)};
    auto dy{S.newNumeric(2, 2)};
    
    auto sz{S.newNumeric()};
    auto z{S.between(sz, sz + 4)};
    auto dz{S.newNumeric(1, 1)};

    auto sw{S.newNumeric()};
    auto w{S.between(sw, sw + 2)};
    auto dw{S.newNumeric(1, 1)};

    auto su{S.newNumeric()};
    auto u{S.between(su, su + 2)};
    auto du{S.newNumeric(2, 2)};

    auto sv{S.newNumeric()};
    auto v{S.between(sv, sv + 1)};
