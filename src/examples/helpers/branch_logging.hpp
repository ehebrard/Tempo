...
namespace tempo {
    ...
    int solve(Solver<T> &S, const Interval<T> &schedule, const std::string &record_file) {
        const auto &opt = S.getOptions();
        if (opt.restart_policy != "no" or opt.primal_boost) {
            throw std::runtime_error("please set restart policy to 'no' and disable primal boost");
        }
