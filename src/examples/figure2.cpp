...
int main() {
  ...
  S.post(t3.end <= schedule.end);*/
    auto schedule = S.newInterval(0, 11, 0, 0, 0, 11);

    auto s1{S.newNumeric()};
    auto t1{S.between(s1, s1 + 3)}; // x 5 = 100
    auto d1{S.newNumeric(1, 1)};

    auto s2{S.newNumeric()};
    auto t2{S.between(s2, s2 + 3)}; // x 10 = 150
    auto d2{S.newNumeric(1, 1)};

    auto s3{S.newNumeric()};
    auto t3{S.between(s3, s3 + 2)}; // x 12 = 120
    auto d3{S.newNumeric(1, 1)};

    auto s4{S.newNumeric()};
    auto t4{S.between(s4, s4 + 3)}; // x 12 = 120
    auto d4{S.newNumeric(1, 1)};
  ...
  S.post(
      Cumulative<int>(schedule, S.newConstant(2), {t1, t2, t3, t4}, {d1, d2, d3, d4}));
  ...
  std::cout << t3 << " (" << d3.min(S) << "): " << t3.start.min(S) << ".."
    ...
    std::cout << t4 << " (" << d4.min(S) << "): " << t4.start.min(S) << ".."
              << t4.end.max(S) << std::endl;
}
