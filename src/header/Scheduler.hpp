...
//    lit getTriggerLiteral(const int) const;
    ...
    void set(const var x, const boolean_state val, Explanation e=Constant::NoReason);
...
//    void incrementActivity(const var x);
    ...
    std::ostream &display(std::ostream &os, const bool dom=true, const bool bra=true, const bool cla=false, const bool egr=false, const bool vgr=false, const bool con=false) const;
...
private:
    ...
    /*
     // the current temporal graph (manages its own reversibility), used to compute the bounds, i.e., the shortest paths from an to the origin
...
//    std::vector<bool> visited_bound;
//    std::vector<T> visited_bound[2];
...
public:
    ...
    bool is_younger(const genlit a, const genlit b) const {
        if(LTYPE(a) == EDGE_LIT) {
            if(LTYPE(b) == EDGE_LIT) {
                ...
                return stamp(FROM_GEN(b)) < static_cast<lit>(getIndex(VAR(FROM_GEN(a))));
            }
        } else {
            if(LTYPE(b) == EDGE_LIT) {
    ...
    [&](const genlit a, const genlit b) {
        return this->is_younger(a,b);
        
//        if(LTYPE(a) == EDGE_LIT) {
//            if(LTYPE(b) == EDGE_LIT) {
//                return search_vars.index(VAR(FROM_GEN(a))) > search_vars.index(VAR(FROM_GEN(b)));
//            } else {
//                return stamp(FROM_GEN(b)) >= search_vars.index(VAR(FROM_GEN(a)));
//            }
//        } else {
//            if(LTYPE(b) == EDGE_LIT) {
//                return stamp(FROM_GEN(a)) < search_vars.index(VAR(FROM_GEN(b)));
//            } else {
//                return a > b;
//            }
//        }
//        
//        
//        if(LTYPE(a) == BOUND_LIT and LTYPE(b) == BOUND_LIT)
//            return a > b;
//        else if(LTYPE(a) == BOUND_LIT)
//            return true;
//        else if(LTYPE(b) == BOUND_LIT)
//            return false;
//        return search_vars.index(VAR(FROM_GEN(a))) > search_vars.index(VAR(FROM_GEN(b)));
////        return level(VAR(FROM_GEN(a))) > level(VAR(FROM_GEN(b)));
////        return LTYPE(a)==BOUND_LIT or level(VAR(FROM_GEN(a))) > level(VAR(FROM_GEN(b)));
    };
...
template<typename T>
Scheduler<T>::Scheduler(Options opt) : ReversibleObject(&env), options(std::move(opt)), domain(*this), clauses(*this), 
evt_constraint_network(&env),
var_constraint_network(&env),
propagation_queue(constraints)
//, decisions(&env), front_propag_pointer(0,&env), back_propag_pointer(0,&env)
, edge_propag_pointer(0,&env), bound_propag_pointer(0,&env)
{
    ...
    if (options.restart_policy == "luby") {
      ...
      restart_policy = new Geometric(options.restart_base, options.restart_factor);
    } else {
...
template<typename T>
size_t Scheduler<T>::numVariable() const {
//    assert(search_vars.capacity() == branch.capacity());
...
template<typename T>
size_t Scheduler<T>::numEdgeLiteral() const {
//    assert((branch.capacity()-branch.size()) == propagation_stack.size());
//    
//    assert(propagation_stack.size() == search_vars.frontsize());
//    
//    return propagation_stack.size(); //branch.capacity()-branch.size();
...
template<typename T>
T Scheduler<T>::getMakespan() const {
    return ub;
}
...
void tempo::Scheduler<T>::propagate() {
    ...
    while(not propagation_queue.empty() or (search_vars.frontsize() > edge_propag_pointer) or (numBoundLiteral() > static_cast<size_t>(bound_propag_pointer))) {
        
        while(search_vars.frontsize() > edge_propag_pointer) {
            ++num_literals;
            lit l{LIT(search_vars[edge_propag_pointer], polarity[search_vars[edge_propag_pointer]])};
//
//            std::cout << " prop: (" << LIT(search_vars[edge_propag_pointer], polarity[edge_propag_pointer]) << ") [" << edges[LIT(search_vars[edge_propag_pointer], polarity[search_vars[edge_propag_pointer]])] << "] <" << polarity[search_vars[edge_propag_pointer]] << ">\n";
...
//    std::cout << "\nlevel of " << prettyLiteral(l) << std::endl;
...
//    std::cout << idx << " / " << search_vars.frontsize() << " / " << search_vars.capacity() << std::endl;
    
    auto x{search_vars[idx]};
    
//    std::cout << " -> " << edges[LIT(x, polarity[x])] << std::endl;
...
void Scheduler<T>::getCriticalPath(std::vector<genlit>& clause) {
    domain.findExplanationPath(HORIZON, ORIGIN, clause, static_cast<lit>(numEdgeLiteral())-1);
}

template<typename T>
void Scheduler<T>::xplain(const lit l, const hint h, std::vector<lit> &Cl) {
    if(l == NoLit) {
//        std::cout << "domain exhausted on var [" << edges[POS(h)] << "]<>[" << edges[NEG(h)] << "]";
...
//            std::cout << "hint: " << prettyLiteral(EDGE(h)) << std::endl;
...
//                    std::cout << " *path* (<= " << domain.bounds.getStamp(FROM_GEN(l)) << ")\n" ;
                    
                    domain.findExplanationPath(x, edges[h].from, Cl, static_cast<lit>(stamp(FROM_GEN(l))));
                }
            }
            
            auto q{domain.bounds.getPastIndex(LOWERBOUND(edges[h].to), FROM_GEN(l))};
//            auto q{BOUND(domain.bounds.getIndex(LOWERBOUND(edges[h].to)))};
    ...
    std::cout << " no need to explain lower bound of " << prettyEvent(edges[h].to) << "\n";
  }
#endif
 
            assert(SIGN(getBoundLiteral(FROM_GEN(BOUND(domain.bounds.getIndex(LOWERBOUND(edges[h].to)))))) == LOWER);
...
//                    std::cout << " *path* (<= " << domain.bounds.getStamp(FROM_GEN(l)) << ")\n" ;
            ...
            auto q{domain.bounds.getPastIndex(UPPERBOUND(edges[h].from), FROM_GEN(l))};
//            auto q{BOUND(domain.bounds.getIndex(UPPERBOUND(edges[h].from)))};
    ...
    std::cout << " no need to explain upper bound of " << prettyEvent(edges[h].from) << "\n";
  }
#endif
        }
...
//    branch.setStart(front_propag_pointer);
//    branch.setEnd(back_propag_pointer);
//    propagation_stack.resize(branch.capacity()-branch.size());
}
...
void Scheduler<T>::notifySolution() {
    ...
    //    best_solution.clear();
    //    for(auto vi{branch.fbegin()}; vi!=branch.fend(); ++vi) {
    //        best_solution.add(*vi);
    //    }
...
void Scheduler<T>::branchRight() {
    ...
    if (DBG_BOUND and (DBG_TRACE & SEARCH)) {
        std::cout << "-- backtrack to lvl " << env.level() << " & deduce " <<  edges[LIT(deduction_var, sign)] << " [i=" << num_choicepoints << "] --\n";
    }
#endif
...
void Scheduler<T>::writeLiteral(const lit l) const {
    auto j{FROM_GEN(l)};
    if(LTYPE(l) == EDGE_LIT) {
        *cl_file << " " << edges[j].from << " " << edges[j].to << " " << edges[j].distance;
    } else {
...
//    domain.bounds.displayTrail(std::cout);
    
    
    analyze(e);
    
    //    assert(max_level < env.level());
    ...
    int max_level{(conflict.size() > 1 ? decisionLevel(conflict[1]) : init_level)};
    ...
    if(num_clauses++ > DBG_CL)
        exit(1);
//    else
//        std::cout << num_clauses << "/" << num_fails << std::endl;
    if(cl_file != NULL) {
...
#ifdef DBG_CL
        if(cl_file != NULL) {
//            auto j{FROM_GEN(conflict[i])};
//            if(LTYPE(conflict[i]) == EDGE_LIT) {
//                *cl_file << " " << edges[j].from << " " << edges[j].to << " " << edges[j].distance;
//            } else {
//                auto c{domain.bounds.getConstraint(j)};
//                auto s{SIGN(c.l)};
...
//    std::cout << "backtrack b/c " << e << std::endl;
//    exit(1);
    ...
    if (DBG_BOUND and (DBG_TRACE & SEARCH)) {
        conflict_set.clear();
        std::cout << "failure @level " << env.level() << "/" << init_level << " b/c " << e << ":\n";
...
void Scheduler<T>::printTrace() const {
    if (DBG_BOUND and (DBG_TRACE & SEARCH)) {
        ...
        display(std::cout, (DBG_TRACE & DOMAINS), (DBG_TRACE & BRANCH), (DBG_TRACE & CLAUSES), false, false, false);
    }
}
#endif
...
bool Scheduler<T>::search() {
    ...
    while (lb < ub and not KillHandler::instance().signalReceived()) {
...
#ifdef DBG_TRACE
        if(DBG_BOUND) {
            std::cout << "--- search node (lvl=" << env.level() << ") [i=" << num_choicepoints << "] ---\n";
            printTrace();
        }
#endif
        ...
        try {
            ...
            // Select an appropriate checkpoint.
...
//                    std::cout << "random (" << num_choicepoints << ")\n";
                } else {
...
//                var cp = branch.any();
 //               lit d = (random()%2 ? POS(cp) : NEG(cp));
                
#ifdef DBG_TRACE
                if (DBG_BOUND and (DBG_TRACE & SEARCH)) {
                    std::cout << "-- new decision: "
                    << edges[d]
                    << std::endl;
                }
#endif
...
template <typename T> bool Scheduler<T>::stoppingCondition() const {
    return conflict_set.size() == 1
            and LTYPE(conflict_set[0]) == EDGE_LIT
    ;
}
...
//    Explanation e{(LTYPE(l) == BOUND_LIT ? domain.bounds.getExplanation(FROM_GEN(l)) : reason[VAR(FROM_GEN(l))])};
//    return e == Constant::NoReason;
    
//        if(LTYPE(l) == BOUND_LIT)
//            return domain.bounds.getReason(FROM_GEN(l)) == Constant::NoReason;
//        return reason[VAR(FROM_GEN(l))] ==
//        decisionLevel(l) == env.level();
}
...
//    auto bl{FROM_GEN(l)};
//    auto c{domain.bounds.getConstraint(bl)};
//    auto s{SIGN(c.l)};
//    auto e{EVENT(c.l)};
//    return visited_bound[s][e] <= c.distance;
}

template <typename T> void Scheduler<T>::markVisited(const genlit l) {
    
//    std::cout << "mark " << prettyLiteral(l) << std::endl;
...
//        auto e{EVENT(c.l)};
//        
//        if(visited_bound[s][e] == INFTY)
//            conflict_bounds.push_back(LIT(e,s));
//        
//        visited_bound[s][e] = c.distance;
        
    }
}

template <typename T> Explanation Scheduler<T>::getExplanation(const genlit l) const {
    return LTYPE(l) == BOUND_LIT ? domain.bounds.getExplanation(FROM_GEN(l)) : reason[VAR(FROM_GEN(l))];
}
...
//  visited.clear();
  conflict.clear();
//    std::vector<genlit>& conflict_set(buffer_lit);
    conflict_set.clear();
...
//          std::cout << e << " (" << prettyLiteral(BOUND(e)) << "): " << domain.bounds.visited[e] << std::endl;
//          
...
//          exit(1);
    ...
    if (DBG_BOUND and (DBG_TRACE & LEARNING)) {
      ...
      for (auto il : conflict_set) {
        std::cout << " "<< prettyLiteral(il);
//          std::cout << " (" << level(VAR(FROM_GEN(il))) << "/" << env.level() << ")";
      }
      std::cout << std::endl;
//        for(auto x : )
    }
...
//    conflict.push_back(clauses.newNegLiteral(conflict_set[0]));
    conflict.push_back(conflict_set[0]);
    
    
//    for(auto l : conflict) {
//        cut.push_back(clauses.newNegLiteral(l))
////        if(LTYPE(l) == EDGE_LIT) {
////            cut.push_back(EDGE(NOT(FROM_GEN(l))));
////        } else {
////            cut.push_back(clauses.newNegLiteral())
////        }
//    }
...
void Scheduler<T>::resolve(const lit l, Explanation e) {
...
#ifdef DBG_TRACE
    if (DBG_BOUND and (DBG_TRACE & LEARNING)) {
        std::cout << "\nresolution step: " << prettyLiteral(l) << " <- " << e << std::endl;
    }
#endif
    ...
    // the literals from previous levels are directly added to the learned conflict
...
//    
    if(cl_file != NULL) {
        ...
        *cl_file << "1 " << (count + (l != NoLit ? 2 : 1)) << " 0 1 " << upper(HORIZON) ;
        
        if(l != NoLit) {
//            *cl_file << (count + 2);
            if(LTYPE(l) == EDGE_LIT) {
                
                //            std::cout << l << " -> " << FROM_GEN(l) << " -> " << NOT(FROM_GEN(l)) << " -> " << EDGE(NOT(FROM_GEN(l))) << " = " << GNOT(l) << std::endl;
                
                assert(GNOT(l) == EDGE(NOT(FROM_GEN(l))));
                
                //            std::cout << prettyLiteral(l) << " / " << prettyLiteral(GNOT(l)) << std::endl;
    ...
    for (auto i{first}; i != last; ++i) {
        ...
        if (needExplanation(*i)) {
#ifdef DBG_TRACE
            if (DBG_BOUND and (DBG_TRACE & LEARNING)) {
                std::cout << ": need further explanation (" << decisionLevel(*i) << "/" << env.level() << ")\n";
            }
#endif
            ...
            *first++ = std::move(*i);
            ...
            //            max_level = std::max(max_level, decisionLevel(*i));
            //            conflict.push_back(clauses.newNegLiteral(*i));
...
//        std::cout << FROM_GEN(el) << " / " << static_cast<lit>(numBoundLiteral()) << std::endl;
        
        assert(FROM_GEN(el) < static_cast<lit>(numBoundLiteral()));
        
//        std::cout << " -- " << el << std::endl;
//        std::cout << " -- " << FROM_GEN(el) << std::endl;
//        std::cout << " -- " << getBoundLiteral(FROM_GEN(el)) << std::endl;
//        return prettyEventLit(getBoundLiteral(FROM_GEN(el)));
        std::stringstream ss;
...
std::ostream &Scheduler<T>::displayConstraints(std::ostream &os) const {
    ...
    for(lit l{0}; l<endBoundLit(); ++l) if(evt_constraint_network.has(l)) {
    ...
    for(lit l{0}; l<endEdgeLit(); ++l) if(var_constraint_network.has(l)) {
...
//    os << "vars:";
//    for(auto i : search_vars)
//    {
//        os << " [" << edges[POS(i)] << "] <> [" << edges[NEG(i)] << "]";
//    }
//    os << "\nprecs:";
    os << "branch:";
...
std::ostream &Scheduler<T>::displayStats(std::ostream &os, const char* msg) const {
    os << msg << ": [" << std::left << std::setw(5) << std::setfill('.') << lb
       << std::setfill(' ');
    if (ub < INFTY)
      os << std::right << std::setw(6) << std::setfill('.') << ub << std::setfill(' ');
    else
      os << ".infty";
  #ifndef DEBUGCMP
    os << "] fails=" << std::setw(7) << std::left
       << num_fails
       << " literals=" << std::setw(12) << std::left
       << num_literals
       << " |cflct|=";
    if (num_fails == 0)
      os << "n/a  ";
...
std::ostream &Scheduler<T>::display(std::ostream &os, const bool dom, const bool bra, const bool cla, const bool egr, const bool vgr, const bool con) const {
    ...
    if(bra) displayBranch(os);
    if(dom) os << domain ;
    if(cla) os << clauses << std::endl;
    if(egr) os << evt_constraint_network << std::endl;
    if(vgr) os << var_constraint_network << std::endl;
    if(con) displayConstraints(os);
    return os;
}
