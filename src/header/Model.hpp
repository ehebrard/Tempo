...
public:
    ...
    NumericVar( NumericVar<T> &&) = default;
    NumericVar &operator=(NumericVar<T> &) = default;
    NumericVar &operator=( NumericVar<T> &&) = default;
...
public:
    ...
    BooleanVar( BooleanVar<T> &&) = default;
    BooleanVar &operator=(BooleanVar<T> &) = default;
    BooleanVar &operator=( BooleanVar<T> &&) = default;
...
public:
    ...
    DisjunctVar( DisjunctVar<T> &&) = default;
    DisjunctVar &operator=(DisjunctVar<T> &) = default;
    DisjunctVar &operator=( DisjunctVar<T> &&) = default;
...
public:
    ...
    TemporalVar( TemporalVar<T> &&) = default;
    TemporalVar &operator=(TemporalVar<T> &) = default;
    TemporalVar &operator=( TemporalVar<T> &&) = default;
...
public:
    ...
    template<typename C>
    void createOrderVariables(C& container);
...
//            container.insert(container.end(), solver.newDisjunct(a->end.before(b->start), b->end.before(a->start)));
            container.push_back(solver.newDisjunct(a->end.before(b->start), b->end.before(a->start)));
        }
    }
}
...
end((mindur == maxdur ? TemporalVar(start.id(), mindur) : solver.newTemporal())),
...
//    std::cout << mindur << ".." << maxdur << std::endl;
  ...
  if (start.id() != end.id()) {
//      std::cout << "\nmindur: " << start.before(end, min_duration) << std::endl;
      solver.set(start.before(end, min_duration));
      
//      std::cout << "\nmaxdur: " << end.before(start, -max_duration) << std::endl;
...
template<typename T>
bool Job<T>::mustExist() const {
    return true;
}

template<typename T>
bool Job<T>::cannotExist() const {
    return false;
}

template<typename T>
T Job<T>::minDuration() const {
    return min_duration;
}

template<typename T>
T Job<T>::maxDuration() const {
    return max_duration;
}

template<typename T>
event Job<T>::getStart() const {
    return start.id();
}

template<typename T>
event Job<T>::getEnd() const {
    return end.id();
}

template<typename T>
std::ostream &Job<T>::display(std::ostream &os) const {
    os << "t" << id() << ": [" << start.earliest(solver) << ".." << end.latest(solver) << "]";
    return os;
}
