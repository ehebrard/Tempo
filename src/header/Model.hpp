...
namespace tempo {
     ...
     */
    template<typename T = int>
    class NumericVar : public InfoStorage<T, NumInfo> {
    public:
        ...
        NumericVar(const var_t i, const T o = 0) noexcept: InfoStorage<T, NumInfo>(
            std::in_place_type<NumInfo<T>>, i, o) {}
     ...
     */
    template<typename T = int>
    class BooleanVar : public InfoStorage<T, BoolInfo> {
    public:
        ...
        index_t semantic() const {
            return (this->isExpression() ? this->implem()->semantic() : this->data()._edge_id_);
        }
    ...
    template<typename T = int>
    class CumulativeExpression;
    template<typename T = int>
    class NoOverlapExpression;
    ...
    template<typename T>
    index_t Interval<T>::num_intervals = 0;
    ...
    public:
      ...
      std::cout << "preprocess\n";
            ...
            if (numeric_arguments.empty()) { return; }
    ...
    BooleanVar<T> operator>(const NumericVar<T> &x, const T k) {
        BooleanVar<T> exp(
            std::make_shared<LeqExpressionImpl<T>>(NumericVar<T>(Constant::K, k + Gap<T>::epsilon()), x, 0));
        return exp;
    }
    ...
    BooleanVar<T> operator>(const T k, const NumericVar<T> &x) {
        BooleanVar<T> exp(
            std::make_shared<LeqExpressionImpl<T>>(x, NumericVar<T>(Constant::K, k + Gap<T>::epsilon()), 0));
        return exp;
    }
    ...
    BooleanVar<T> BigAnd(Iterable &X) {
    ...
    public:
        template<typename Iter>
        LogicalOrExpressionImpl(Iter beg_var, Iter end_var) {
            for (auto x{beg_var}; x != end_var; ++x) {
                boolean_arguments.emplace_back(*x);
            }
        }
    ...
    //TODO decuce T
    template<typename T>
    BooleanVar<T> BigOr(const std::vector<BooleanVar<T>> &X) {
    ...
    public:
        ...
        void post(Solver<T> &solver) override {
            ...
            if (std::distance(this->begin(), this->end()) > 1) {
                ...
                if (solver.getOptions().transitivity) {
                    ...
                    solver.postTransitivity(schedule, std::ranges::subrange(this->begin(), this->end()),
                                            disjunctiveLiterals);
                }
            }
        }
        ...
        void setTransisions(const Matrix &D) {
    ...
    template<typename T>
    class NoOverlapExpression : public BooleanVar<T> {
    public:
            ...
            for (auto [taskIdxA, intervalA, demandA]: const_zip_enumerate(*this, demand)) {
                for (auto [taskIdxB, intervalB, demandB]: const_zip_enumerate(*this | drop(taskIdxA + 1),
                    ...
                    if (intervalA.isOptional(solver) or intervalB.isOptional(solver)) {
                        ...
                        disjunctiveLiterals(taskIdxA, taskIdxB) = solver.boolean.getLiteral(true, x_anb);
                        // a is not before b
                        disjunctiveLiterals(taskIdxB, taskIdxA) = solver.boolean.getLiteral(true, x_bna);
                        // b is not before a
                        solver.addToSearch(x_ab);
                        solver.addToSearch(x_ba);
    ...
    template<typename T>
    class CumulativeExpression : public BooleanVar<T> {
    public:
        ...
        const auto &getDisjunctiveLiterals() const noexcept {
            return static_cast<const CumulativeExpressionImpl<T> *>(BooleanVar<T>::implem().get())->
    ...
    */
    ...
    template<typename T>
    Literal<T> NumericVar<T>::after(const T t) const {
        return geq<T>(id(), (t == Constant::Infinity<T> ? t : t - offset()));
    }

    template<typename T>
    Literal<T> NumericVar<T>::before(const T t) const {
    ...
    */
    ...
    template<typename T>
    bool Interval<T>::operator==(const Interval<T> &t) const {
        return id() == t.id();
    }
    ...
    template<typename T>
    bool Interval<T>::mustExist(Solver<T> &) const {
        return true;
    }

    template<typename T>
    bool Interval<T>::cannotExist(Solver<T> &) const {
        return false;
    }
    ...
    template<typename T>
    var_t Interval<T>::getStart() const { return start.id(); }

    template<typename T>
    var_t Interval<T>::getEnd() const { return end.id(); }
