...
template <typename T = int> struct Datapoint {
    ...
    Datapoint() {}
    Datapoint(const T overflow, const T consumption, const T overlap, const T slackUnder, const T available)
    : overflow(overflow), consumption(consumption), overlap(overlap), slackUnder(slackUnder), available(available) {}
...
public:
    ...
    T scheduleOmega(const T C, const int i, const T max_lct, const bool adjustment=false);
...
template <typename T> void CumulativeEdgeFinding<T>::setLeftCutToTime(const T t) {
    if(leftcut_pointer < static_cast<int>(lct_order.size()) and lct(lct_order[leftcut_pointer]) < t) {
        growLeftCutToTime(t);
    } else {
...
//        std::cout << " - remove " << prettyTask(lct_order[leftcut_pointer]) << std::endl;
...
template <typename T>
T CumulativeEdgeFinding<T>::scheduleOmega(const T C, const int i, const T max_lct, const bool adjustment) {
...
#ifdef DBG_SEF
            if (DBG_SEF) {
                std::cout << "contact[" << task[i].id() << "] = " << profile[contact[i]] << " (" << contact[i] << ") contact time = " << profile[contact[i]].time << std::endl;
            }
#endif
        }
    }
...
template <typename T> void CumulativeEdgeFinding<T>::forwardAdjustment() {
    ...
    if (DBG_SEF) {
        if(in_conflict.empty())
            std::cout << "\nno forward adjustement\n";
        else
            std::cout << "\nstart forward adjustement\n";
    }
#endif
    ...
    while(not in_conflict.empty()) {
        ...
        scheduleOmegaAdjustment(capacity.max(m_solver), i, lct(j), true);
    ...
    while(k >= 0) {
        ...
        while(k >= leftcut_pointer) {
            ...
            if (beta != -1) {
#ifdef DBG_SEF
                if (DBG_SEF) {
                    std::cout << "  - beta = " << beta << " (task " << task[beta].id() << ")" << std::endl;
                }
#endif
                ...
                addPrime(i, beta);
                auto ect_i_H = scheduleOmega(cap, i, lct(beta));
                rmPrime(i, beta);
                ...
                if (ect_i_H > lct(beta)) {
                    ...
                    if (DBG_SEF) {
                        std::cout
                        << " task " << task[i].id() << " is in conflict with task interval "
                        << task[*(lct_order.begin())].id() << ".." << task[beta].id() << std::endl;
                    }
#endif
...
#ifdef DBG_SEF
                if (DBG_SEF) {
                    std::cout << "  - alpha = " << alpha << " (task " << task[alpha].id() << ")" << std::endl;
                }
#endif
...
//                std::cout << profile << std::endl;
                ...
                addPrime(i, alpha);
                auto ect_i_H = scheduleOmega(cap, i, lct(alpha));
                rmPrime(i, alpha);
                ...
                if (ect_i_H > lct(alpha)) {
                    ...
                    if (DBG_SEF) {
                        std::cout
                        << " task " << task[i].id() << " is in conflict with task interval "
                        << task[*(lct_order.begin())].id() << ".." << task[alpha].id() << std::endl;
                    }
#endif
