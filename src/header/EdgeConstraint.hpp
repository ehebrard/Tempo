...
template <typename T>
EdgeConstraint<T>::EdgeConstraint(Scheduler<T> &scheduler,
                            const lit p_)
...
//        m_schedule.set(p, {this, static_cast<hint>(m_schedule.numBoundLiteral())});
//
//    // never propagate
//    return false;
//}
...
bool EdgeConstraint<T>::notify_bound(const lit, const int) {
    ...
    if(DBG_EDGECONS) {
        ...
        << " & "
        << prettyEvent(edge.from) << " >= " << m_schedule.lower(edge.from)
        << " <= " << edge.distance << "\n";
    }
#endif
...
#ifdef DBG_EDGECONS
            if(DBG_EDGECONS) {
                std::cout << " ==> " << m_schedule.prettyLiteral(EDGE(p)) << " (" << r_lb << "/" << r_ub << ")\n";
...
template <typename T>
void EdgeConstraint<T>::xplain(const lit, const hint,
                                         std::vector<lit>& Cl) {
////    if(l == NoLit) {
////        "?";
////    } else {
